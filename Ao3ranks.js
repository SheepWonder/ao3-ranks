...
async function searchPublicRecLists(searchTerm) {
    ...
    try {
        ...
        // Then, group fics by list owner (lower priority)
        const ficsByOwner = {};
        ficsResponse.fics.forEach(fic => {
            // Skip if we already have this list from direct list search
            const existingList = searchResults.find(result => 
                result.username === fic.owner_username && 
                result.listName === fic.list_name
            );
            
            if (!existingList) {
                if (!ficsByOwner[fic.owner_username]) {
                    ficsByOwner[fic.owner_username] = {
                        type: 'fic',
                        username: fic.owner_username,
                        listName: fic.list_name,
                        fics: [],
                        isListMatch: false
                    };
                }
                ...
                // Convert API format to display format
                ficsByOwner[fic.owner_username].fics.push({
                    workId: fic.ao3_url.match(/\/works\/(\d+)/)?.[1] || 'unknown',
                    url: fic.ao3_url,
                    title: fic.title,
                    author: fic.author,
        ...
        // Add fic-based results after list-based results
        Object.values(ficsByOwner).forEach(ownerData => {
            searchResults.push({
                type: 'fic',
                username: ownerData.username,
                listName: ownerData.listName,
                fics: ownerData.fics,
                totalFics: ownerData.fics.length,
                isListMatch: false
            });
        });
...
function displaySearchResults(searchTerm, results) {
    ...
    let resultsHTML = `
        ...
        <div class="search-results-container">
            ...
            <p class="results-summary">Found ${listMatches.length} matching lists and ${totalFics} matching fics</p>
    `;
